...
function cart_realod(cart_ids) {
    ...
    for (let item of temp) {
        ...
        let  stariy_p = document.createElement('p')
        stariy_p.innerHTML = `${item.price} usd`
        details_div.append(stariy_p)
        stariy_p.style.marginLeft = "200px"
        stariy_p.style.display = "none"
        ...
        counter_minus.onclick = () => {
            counter_input.value --
            stariy_p.style.display = "flex"
            if(counter_input.value < 0 ) {
                counter_input.value  = 0
                
            }
            
             let defoult_price = item.price    
            right_price.innerHTML = (item.price - item.price ) + "usd"
        ...
        counter_plus.onclick = () => {
            ...
            stariy_p.style.display = "flex"
            new_price ++
            right_price.innerHTML = new_price  + "usd"
            if(counter_input.value > 10 ) {
                counter_input.value  = 0
